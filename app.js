...
async function saveWatch() {
  ...
  try {
    if (id) {
      ...
      const result = await apiFetch('/api/watches', { method: 'POST', body: JSON.stringify(data) });
      showToast('Watch added successfully.');
      // Check for CRM request matches
      if (result && result.crm_matches && result.crm_matches.length > 0) {
        showCrmMatchAlert(result.crm_matches, data.brand, data.model);
      }
    }
    closeModal();
    refreshCurrentView();
      ...
      rows.push(`<tr class="package-header-row" onclick="event.stopPropagation()">
        <td colspan="7">
          <div class="package-header">
            ...
            <button class="btn btn-primary btn-xs" onclick="event.stopPropagation();openPackageStatusModal('${escapeHtml(pkgId)}')" title="Update status of all watches" style="margin-left:auto;">Update Status</button>
            <button class="btn btn-secondary btn-xs" onclick="event.stopPropagation();downloadPackageInvoice('${escapeHtml(pkgId)}')" title="Download Package Invoice">Invoice</button>
            <button class="btn btn-secondary btn-xs" onclick="event.stopPropagation();copyPackageTrackingLink('${escapeHtml(pkgId)}')" title="Copy Package Tracking Link">Copy Link</button>
          </div>
        </td>
      </tr>`);
    }
...
async function openDealDetail(dealId) {
  try {
    ...
    footer.innerHTML = `
      ...
      ${(d.package_id && d.package_id.startsWith('PKG-')) ? `<button class="btn btn-secondary" onclick="downloadPackageInvoice('${d.package_id}')">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-2px;margin-right:4px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
        Package Invoice
      </button>
      <button class="btn btn-secondary" onclick="copyPackageTrackingLink('${d.package_id}')">
      ...
      ${!d.approved ? `
        ...
        ${(d.package_id && d.package_id.startsWith('PKG-')) ? `<button class="btn btn-secondary" onclick="closeDealDetailModal();openPackageStatusModal('${d.package_id}')">Update All in Package</button>` : ''}
...
// Download package invoice
async function downloadPackageInvoice(pkgId) {
  try {
    showToast('Generating package invoice...');
    const resp = await fetch(`${API}/api/deals/package/${pkgId}/invoice`);
    if (!resp.ok) throw new Error('Failed to generate invoice');
    const blob = await resp.blob();
    const blobUrl = URL.createObjectURL(blob);
    window.open(blobUrl, '_blank');
    ...
    const cd = resp.headers.get('content-disposition') || '';
    const match = cd.match(/filename="?([^"]+)"?/);
    a.download = match ? match[1] : `Invoice_${pkgId}.pdf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    setTimeout(() => URL.revokeObjectURL(blobUrl), 10000);
    showToast('Package invoice ready', 'success');
  } catch(e) {
    console.error('Package invoice error:', e);
    showToast('Failed to download package invoice', 'error');
  }
}
...
function openPackageStatusModal(pkgId) {
  document.getElementById('statusPackageId').value = pkgId;
  // Find all active deals in this package to determine current status
  const pkgDeals = allDeals.filter(d => d.package_id === pkgId && !d.approved);
  if (pkgDeals.length === 0) {
    showToast('No active deals in this package.', 'error');
    return;
  }
  // Show info about what's in the package
  const info = document.getElementById('packageStatusInfo');
  info.innerHTML = pkgDeals.map(d => {
    const w = [d.watch_brand, d.watch_reference].filter(Boolean).join(' ') || 'Unknown';
    return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:var(--text-sm);border-bottom:1px solid var(--color-border);">
      <span style="flex:1;font-weight:500;">${escapeHtml(w)}</span>
      <span class="${getDealBadgeClass(d.status)}" style="font-size:10px;">${DEAL_STATUS_LABELS[d.status] || d.status}</span>
    </div>`;
  }).join('');
  // Default the select to the next status from the most common current status
  const statusCounts = {};
  pkgDeals.forEach(d => { statusCounts[d.status] = (statusCounts[d.status] || 0) + 1; });
  const mostCommon = Object.entries(statusCounts).sort((a, b) => b[1] - a[1])[0][0];
  const currentIdx = DEAL_STATUS_ORDER.indexOf(mostCommon);
  const nextStatus = currentIdx < DEAL_STATUS_ORDER.length - 1 ? DEAL_STATUS_ORDER[currentIdx + 1] : mostCommon;
  document.getElementById('statusPackageSelect').value = nextStatus;
  document.getElementById('packageStatusModal').classList.add('open');
}
function closePackageStatusModal() {
  document.getElementById('packageStatusModal').classList.remove('open');
}
async function confirmPackageStatusUpdate() {
  const pkgId = document.getElementById('statusPackageId').value;
  const status = document.getElementById('statusPackageSelect').value;
  try {
    const result = await apiFetch(`/api/deals/package/${pkgId}/status`, {
      method: 'PUT',
      body: JSON.stringify({ status })
    });
    showToast(`${result.updated_count} watches updated to ${DEAL_STATUS_LABELS[status]}`, 'success');
...
async function saveBulkWatches() {
  ...
  for (const data of payloads) {
    try {
      ...
      successCount++;
      if (result && result.crm_matches && result.crm_matches.length > 0) {
        result.crm_matches.forEach(m => {
          if (!allCrmMatches.find(x => x.id === m.id)) {
            allCrmMatches.push(m);
          }
        });
      }
    } catch(e) {
  ...
  // Show CRM match alert if any
  if (allCrmMatches.length > 0) {
    showCrmMatchAlert(allCrmMatches, 'Bulk Add', '');
  }
}
...
function showCrmMatchAlert(matches, watchBrand, watchModel) {
  const modal = document.getElementById('crmMatchModal');
  const body = document.getElementById('crmMatchBody');
  const watchLabel = watchModel ? `${watchBrand} ${watchModel}` : watchBrand;

  body.innerHTML = `
    <div style="margin-bottom:var(--space-4);color:var(--color-text-muted);font-size:var(--text-sm);">
      The watch you just added matches ${matches.length} client request${matches.length > 1 ? 's' : ''}:
    </div>
    ${matches.map(m => `
      <div class="crm-match-item">
        <div class="crm-match-item-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
        </div>
        <div class="crm-match-item-info">
          <div class="crm-match-item-name">${escapeHtml(m.client_name)}</div>
          <div class="crm-match-item-detail">Looking for: <strong>${escapeHtml(m.watch_requested)}</strong></div>
          ${m.budget ? `<div class="crm-match-item-detail">Budget: ${formatCurrency(m.budget)}</div>` : ''}
          ${m.client_contact ? `<div class="crm-match-item-detail">${escapeHtml(m.client_contact)}</div>` : ''}
        </div>
...
function closeCrmMatchModal() {
  document.getElementById('crmMatchModal').classList.remove('open');
}

function goToRequestsFromMatch() {
  closeCrmMatchModal();
  switchView('crm');
}

async function loadCrmMatchBanner() {
  const banner = document.getElementById('crmMatchBanner');
  if (!banner) return;
  try {
    const matches = await apiFetch('/api/crm/matches');
    if (!matches || matches.length === 0) {
      banner.style.display = 'none';
      return;
    }
    banner.style.display = '';
    const totalWatches = matches.reduce((sum, m) => sum + (m.matched_watches ? m.matched_watches.length : 0), 0);
    document.getElementById('crmMatchBannerCount').textContent = matches.length;
    banner.querySelector('.crm-match-banner-text').textContent =
      `${matches.length} request${matches.length > 1 ? 's' : ''} match${matches.length === 1 ? 'es' : ''} watches in your inventory`;

    const list = document.getElementById('crmMatchBannerList');
    list.innerHTML = matches.map(m => {
      const watchNames = (m.matched_watches || []).map(w => `${w.brand} ${w.model}`).join(', ');
      return `
        <div class="crm-match-banner-item">
          <div class="crm-match-banner-item-info">
            <strong>${escapeHtml(m.client_name)}</strong>
            <span class="crm-match-banner-sep">→</span>
            <span>${escapeHtml(m.watch_requested)}</span>
          </div>
          <div class="crm-match-banner-item-watches">
            <span class="badge badge-match" style="font-size:10px;padding:1px 6px;">In Stock: ${escapeHtml(watchNames)}</span>
          </div>
        </div>
      `;
    }).join('');
...
// Enhance CRM table to show match indicators
const _originalRenderCrmTable = renderCrmTable;
renderCrmTable = async function(requests) {
  _originalRenderCrmTable(requests);
  // After rendering, fetch matches and annotate rows
  try {
    const matches = await apiFetch('/api/crm/matches');
    const matchedIds = new Set(matches.map(m => m.id));
    const rows = document.querySelectorAll('#crmBody tr');
    const reqArray = requests || [];
    reqArray.forEach((r, i) => {
      if (matchedIds.has(r.id) && rows[i]) {
        const statusCell = rows[i].querySelector('td:first-child');
        if (statusCell && !statusCell.querySelector('.match-dot')) {
          const matchWatches = matches.find(m => m.id === r.id);
          const watchNames = matchWatches && matchWatches.matched_watches
            ? matchWatches.matched_watches.map(w => `${w.brand} ${w.model}`).join(', ')
            : 'inventory';
          statusCell.insertAdjacentHTML('beforeend',
            `<span class="match-dot" title="Matches in stock: ${escapeHtml(watchNames)}">⚡</span>`);
        }
        rows[i].classList.add('crm-row-matched');
      }
    });
  } catch(e) {}
  // Load the match banner too
  loadCrmMatchBanner();
};
